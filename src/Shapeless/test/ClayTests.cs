...
public class ClayTests(ITestOutputHelper output)
{
    ...
    public void DeserializeNode_ReturnOK()
    {
        ...
        var obj4 = Clay.DeserializeNode(JsonNode.Parse("\"2024/12/29 3:34:30\""),
            new ClayOptions { DateJsonToDateTime = true });
    ...
    [Fact]
    public void TryConvertKeyValueJsonToObject_ReturnOK()
    {
        var jsonNodeOptions = new JsonNodeOptions();
        var jsonDocumentOptions = new JsonDocumentOptions();

        Assert.False(Clay.TryConvertKeyValueJsonToObject(null, jsonNodeOptions, jsonDocumentOptions,
            out var jsonObject1));
        Assert.Null(jsonObject1);

        Assert.False(Clay.TryConvertKeyValueJsonToObject(JsonNode.Parse("{}"), jsonNodeOptions,
            jsonDocumentOptions,
            out var jsonObject2));
        ...
        Assert.False(Clay.TryConvertKeyValueJsonToObject(JsonNode.Parse("[1,2]"), jsonNodeOptions,
            jsonDocumentOptions,
            out var jsonObject4));
        Assert.Null(jsonObject4);

        Assert.False(Clay.TryConvertKeyValueJsonToObject(JsonNode.Parse("""
                                                                        [
                                                                          {
                                                                            "key": "id",
                                                                            "value": 1
        ...
        Assert.True(Clay.TryConvertKeyValueJsonToObject(JsonNode.Parse("""
                                                                       [
                                                                         {
                                                                           "key": "id",
                                                                           "value": 1
    ...
    [Fact]
    public void ExecuteTransformation_Invalid_Parameters()
    {
        var clay = Clay.Parse("""{"id":1,"name":"furion"}""");
        Assert.Throws<ArgumentNullException>(() => clay.ExecuteTransformation(null!, true));
        Assert.Throws<ArgumentNullException>(() => clay.ExecuteTransformation(null!, false));

        var exception = Assert.Throws<InvalidOperationException>(() => clay.ExecuteTransformation(u => u.name, true));
        Assert.Equal(
    ...
    [Fact]
    public void ExecuteTransformation_ReturnOK()
    {
        const string jsonString = """
                                  ...
                                  """;
        var (_, enumerable) = Clay.Parse(jsonString).ExecuteTransformation(u => u.data, true);
        Assert.Equal(["二手小米SU7 Ultra卖到65万", "加拿大省长模仿特朗普签令下架美国酒"], enumerable.Select(u => u!.title));

        var (_, enumerable2) = Clay.Parse(jsonString).ExecuteTransformation(u => u.data2, false)
            .ExecuteTransformation(u => u.data, true);
        Assert.Equal(["二手小米SU7 Ultra卖到65万", "加拿大省长模仿特朗普签令下架美国酒"], enumerable2.Select(u => u!.title));
    }
}
